...
x_kspace = masked_kspace_1.ravel('K')
testOP = Pad_Op.H * Sop * Pad_Op * Fop_3D.H
...
(x1, niter1, cost1) = pylops.optimization.sparsity.fista(Op, y1, eps=epsilon, x0=x0_1, niter=Iteration_number, SOp=Sop,show=True,alpha=0.2, threshkind='soft')
(x2, niter2, cost2) = pylops.optimization.sparsity.fista(Op, y2, eps=epsilon, x0=x0_2, niter=0, SOp=Sop, tol=Tolerance,show=True,alpha=0.5)
...
nifti_img = nib.Nifti1Image(abs(recons_avg), affine=None)
nib.save(nifti_img, f"{nifpath}/{"image2"}_{data_name}_{Wavelet_3D}_Reg={epsilon}.nii")
...
abs_recons_all = np.abs(recons_avg)
...
for i in range(len(recons_avg)):
    ...
    recon_image = (np.abs(recons_avg[i]) / i_max * 255).astype(np.uint8)
    original_image = (np.abs(images[i]) / i_max * 255).astype(np.uint8)

    Image.fromarray(recon_image).save(f"{Output_File}/reconstructed_{i}.png")
    Image.fromarray(original_image).save(f"{Output_File}/original_{i}.png")
    Image.fromarray((colour_diff[:,:,:3]*255).astype(np.uint8)).save(f"{Output_File}/difference_{i}.png")##
...
comparison_slices = [0, len(recons_avg)//2, len(recons_avg)-1] + list(range(0, len(recons_avg), 10))
...
for i in comparison_slices:
    ...
    axs[1, 2].axis('off')
    plt.savefig(f"{Output_File}/comparison_{i}.png", dpi=100, bbox_inches='tight')
    plt.close(fig)
...
plt.yscale('log')
plt.ylabel('Cost')
plt.savefig(f"{Output_File}/cost_function.png")
plt.close()

#save the cost function values to a text file
with open(f"{Output_File}/cost_values.txt", 'w') as f:
    for cost in cost1:
        f.write(f"{cost}\n")

##save a text file with the final data fidelity and regularization terms
final_data_fidelity = np.linalg.norm(Op * x1 - y1)**2
final_regularization = np.linalg.norm(Sop.H * x1, 1)
with open(f"{Output_File}/final_terms.txt", 'w') as f:
