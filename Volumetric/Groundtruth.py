...
images1 = Fop_3D_ifft.H * kspace1.ravel('K')
images2 = Fop_3D_ifft.H * kspace2.ravel('K')
images1 = images1.reshape((nl, ny, nx))
images2 = images2.reshape((nl, ny, nx))
...
images1 = images1.astype(np.complex128)
images2 = np.asarray(images2)
images2 = images2.astype(np.complex128)
kspace1 = np.asarray(kspace1)
kspace1 = kspace1.astype(np.complex128)
kspace2 = np.asarray(kspace2)
kspace2 = kspace2.astype(np.complex128)
...
folderpath_basename = os.path.basename(folderpath)
print(f"Folder path: {folderpath}, Basename: {folderpath_basename}")
###generate a nifti image of the reconstructed stack
nifti_img = nib.Nifti1Image(abs(images1), affine=None)
nib.save(nifti_img, f"{Output_File}/images_{folderpath_basename}_1.nii")
nifti_img = nib.Nifti1Image(abs(images2), affine=None)
nib.save(nifti_img, f"{Output_File}/images_{folderpath_basename}_2.nii")
