...
$root = "C:\Users\osman\Documents\FYP Datasets\Example image"
$samples = "C:\Users\osman\Documents\FYP Datasets\Batch 7 Sample"
...
$run_recon = 1
if ($run_recon -eq 1) {
    foreach ($samplingfile in $samplingfiles) {
        ...
        for ($i = 0; $i -lt $deepestDirnames.Count; $i++) {
            ...
            $currentoutput = "$resultspath\$currentdirname _ $samplingname Results 2"
            Write-Host "Current output path: " + $currentoutput

            New-Item -Path $currentoutput -ItemType Directory

            & python .\Volumetric\Volumetric_kspace_no_resize.py $currentpath $target_size $Wavelet_name $Wavelet_level $Regularization_parameter $Iteration_number $Tolerance $currentoutput $samplingpath $niftipath
        }
    }
}
