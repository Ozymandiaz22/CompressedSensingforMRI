...
outputdir = os.path.join(outputdir, 'exLUT_radial_trajectories')
...
for targetPercent in samplePercents:
    ...
    if exportList:
            ...
            filename = os.path.join(outputdir, f'exLUT_radial_y{dimy}_z{dimz}_pct{targetPercent}_a{round(tinyGoldenAngles[angleNr-1],2)}_r{reps}.txt')
    ...
    print('----------------------------------\n')
    coveragetotal = 100 * numUnique / totalGridPoints
    ...
    plt.imsave(os.path.join(outputdir, f'exLUT_radial_trajectory_y{dimy}_z{dimz}_pct{coveragetotal:.1f}_a{round(tinyGoldenAngles[angleNr-1],2)}_r{reps}.bmp'), trajectoryImage, cmap='gray', format='bmp')